...
@if ($message = Session::get('success'))
    <div class="toast-container position-fixed top-0 end-0 p-3" style="z-index: 1100;">
        <div class="toast align-items-center text-bg-success border-0 shadow" id="successToast" role="alert"
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table id="example" class="table table-bordered table-responsive">
                        ...
                        <tbody>
                            @forelse ($influencer as $influencerData)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if (filter_var(optional($influencerData->influencerProfile)->is_featured, FILTER_VALIDATE_BOOLEAN))
                                    ...
                                    {{-- Trending --}}
                                    <td class="text-center">
                                        @if (filter_var(optional($influencerData->influencerProfile)->is_trending, FILTER_VALIDATE_BOOLEAN))
                                            <i class="bi bi-check text-success h2"></i>
                                        @else
                                            <i class="bi bi-x text-danger h2"></i>
                                        @endif
                                    ...
                                    {{-- BrandBeans Verified --}}
                                    <td class="text-center">
                                        @if (filter_var(optional($influencerData->influencerProfile)->is_brandBeansVerified, FILTER_VALIDATE_BOOLEAN))
                                            <i class="bi bi-check text-success h2"></i>
                                        @else
                                            <i class="bi bi-x text-danger h2"></i>
                                        @endif
                                    ...
                                    <td>
                                        <a href="{{ route('influencer.singleView', $influencerData->id) }}"
                                            class="btn btn-primary btn-sm">View Details</a>
                                        <a href="{{ route('influencer.statusEdit', $influencerData->id) }}"
                                            class="btn btn-info btn-sm">Edit</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center">No Influencers Found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
